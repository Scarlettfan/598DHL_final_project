...
def train():
    ...
    f = open("FNN_mortality_output_cpu.txt", "a")
    ...
    sizedata = len(data_x)
    f.write("Data of size:"+ str(sizedata)+"\n")
    ...
    for ind_i in range(0, k):
        ...
        f.write("Training done - deleting variables and starting test"+"\n")
        dataset = None
        train_loader = None
        tensor_x = None
        tensor_y = None
        ...
        f.write("ROC Average Score:" + str(ROC_avg_score)+"\n")
        AUC_folds.append(ROC_avg_score)

    # Output score of each fold + average
    f.write("AUC_scores: "+ str(AUC_folds)+"\n")
    f.write("Mean: "+ str(sum(AUC_folds)/len(AUC_folds))+"\n")
    f.close()
